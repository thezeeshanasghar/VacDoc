...
filterCountryCodes(value: string) {
  ...
  public filter(value: string) {
    if (!value.trim()) {
      ...
      this.cities = this.originalCities.filter(
        (option) =>
  ...
  sendMessage(sms1: string): void {
    ...
    const accessToken =
      "EAANxUIaDgugBO5XJ4tQLZBHcRQgF7l9znlMHTl1QXgjx4WXmMjF1J7hqAnFdQxUOOgsc7YMhj7FvBRWG60QdMNitVTeaZAI49YidLNZB2dtfKRDyBYOY28hQUYkvxg2hvqCrOvupbAGPfcC3ZBDSejZAv8ZBxts3qTjsh1tN8TCiqEwKYqGuaMPKA4wPkCzUzteXjbCSCOvgP5gdN2rC8ZD";
      ...
      async (res) => {
        ...
        if (res.IsSuccess) {
          ...
          if (res.ResponseData.Gender == "Boy")
            sms1 +=
              "Mr. " + this.titlecasePipe.transform(res.ResponseData.Name);
          ...
          const whatsappNumber =
            "+" + res.ResponseData.CountryCode + res.ResponseData.MobileNumber;
          ...
          const alert = await this.alertCtrl.create({
            ...
            message:
              "Would you like to open WhatsApp to send the registration details?",
            buttons: [
              {
                text: "Cancel",
                role: "cancel",
        ...
        (res) => {
          if (res.IsSuccess) {
            ...
            this.toastService.create(
              "Clinic set as online successfully",
              "success"
            );
  ...
  validation_messages = {
    name: [
      ...
      {
        type: "pattern",
        message: "Please enter only characters in the first name.",
      },
    ],
    City2: [
    ...
    fatherName: [
      { type: "required", message: "Guardian name is required." },
      {
        type: "pattern",
        message:
          "Only letters, spaces, commas, and hyphens are allowed in Guardian.",
      },
    ],
    ...
    nationality: [
      ...
      {
        type: "pattern",
        message: "Please enter a valid nationality (letters and spaces only).",
      },
    ],
  };
}