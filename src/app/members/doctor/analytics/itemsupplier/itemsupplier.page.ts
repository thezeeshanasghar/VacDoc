...
@Component({
  ...
  templateUrl: './itemsupplier.page.html',
  styleUrls: ['./itemsupplier.page.scss'],
})
export class ItemSupplierPage implements OnInit {
  clinics: any[] = [];
  ...
  async getItemsReport() {
    ...
    try {
      this.stockService.getItemsSupplierReportFile(payload.clinicId, payload.Supplier, payload.fromDate, payload.toDate).subscribe({
        next: (response) => {
          loading.dismiss();
          const blob = new Blob([response], { type: 'application/pdf' });
          const link = document.createElement('a');
          link.href = window.URL.createObjectURL(blob);
          link.download = `ItemsReport_${payload.clinicId}_${payload.Supplier}_${payload.fromDate}_${payload.toDate}.pdf`;
          link.click();
