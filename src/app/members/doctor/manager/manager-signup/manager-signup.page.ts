...
@Component({
  ...
  templateUrl: './manager-signup.page.html',
  styleUrls: ['./manager-signup.page.scss'],
})
export class ManagerSignupPage implements OnInit {
  ...
  ngOnInit() {
    this.fg = this.formBuilder.group({
      ...
      // Required (not optional like PA's) - a Manager with no clinic access can't
      // register patients or direct any PA, so there's nothing for the account to do.
  ...
  async submit() {
    ...
    this.managerService.signUpManager(managerData).subscribe({
      next: (res) => {
        if (res.IsSuccess) {
          const newManagerId = res.ResponseData && res.ResponseData.Id ? res.ResponseData.Id : null;
          const clinicIds: any[] = this.fg.value.ClinicId || [];

          if (newManagerId && clinicIds.length > 0) {
            let completed = 0;
            let failed = 0;
            clinicIds.forEach((clinicId) => {
              this.managerService.addManagerAccess({ ManagerId: newManagerId, ClinicId: clinicId }).subscribe({
                next: () => {
                  completed++;
                  if (completed + failed === clinicIds.length) {
                    loading.dismiss();
                    const msg = failed > 0
                      ? 'Manager added, some clinic access assignments failed.'
                      : 'Manager added with clinic access.';
                ...
                error: () => {
                  ...
                  if (completed + failed === clinicIds.length) {
                    loading.dismiss();
                    this.toastService.create('Manager added, some clinic access assignments failed.', 'warning');
                    this.router.navigate(['/members/doctor/manager']);
                  }
                }
              });
            ...
            loading.dismiss();
            this.toastService.create('Manager added! Approve and set permissions from the list.', 'success');
            this.router.navigate(['/members/doctor/manager']);
          }
        } else {
          loading.dismiss();
